...
class TvController extends Controller
{
    ...
    {
        // Find the TV by its screen_id
        $tv = Tv::where('screen_id', $id)->first();

        if (!$tv) {
            return response()->json(['message' => 'TV not found'], 404);
        }

        // Update the TV's status to online (1)
        $tv->update(['status' => 1]);

        return response()->json(['message' => 'TV status updated successfully'], 200);
    }
}
